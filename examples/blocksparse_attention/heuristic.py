...
def num_splits_heuristic(total_mblocks, num_SMs, num_n_blocks):
    """
...
def num_splits_heuristic_fast_cached(total_mblocks, num_SMs, num_n_blocks):
    if total_mblocks >= 0.8 * num_SMs or num_n_blocks <= 4:
        return 1
    ...
    for n in range(1, max_splits + 1):
        waves = total_mblocks * n / num_SMs
        eff = waves / math.ceil(waves)
        if eff > max_eff:
            max_eff = eff
