...
def ref_program(Q, K, V, is_causal):
    ...
    return flash_attn_interface.flash_attn_func(Q, K, V, causal=is_causal)[0]
...
def main(
    ...
    if (not tune):
        ...
        latency = profiler.do_bench(ref_program_processed, warmup=500, n_repeat=100)
        print("Ref: {:.2f} ms".format(latency))
        print("Ref: {:.2f} TFlops".format(total_flops / latency * 1e-9))
        latency = profiler.do_bench(warmup=500, n_repeat=100)
